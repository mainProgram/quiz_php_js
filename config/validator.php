...
function is_mail_valid(string $key, string $email, array &$errors, string $message="Invalid mail adress!"){
    if(!preg_match("/^([a-zA-Z0-9]+)([\.{1}])?([a-zA-Z0-9]+)\@gmail([\.])com$/",$email))    
        $errors[$key] = $message;
}
...
function is_extension_valid(string $key, string $extension, array &$errors, string $message="Invalid file !"):void{
